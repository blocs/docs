...
class Excel
{
    ...
    public function get($sheetNo, $sheetColumn, $sheetRow, $formula = false)
    {
        ...
        // 指定されたセルの値または式を取得する
        $value = $this->extractCellValue($worksheetXml, $sheetName, $columnName, $rowName, $formula);
    ...
    private function extractCellValue($worksheetXml, $sheetName, $columnName, $rowName, $formula = false)
    {
            ...
            foreach ($row->c as $cell) {
                if ($cell['r'] == $cellName) {
                    if ($formula) {
                        ...
                        return $this->resolveSheetFormula($worksheetXml, $sheetName, $cell, $cellName);
                    }
                    if ($cell['t'] == 's') {
